...
dataname="seedling_segmentation"

patch_size = 1268 #size of the tiles to extract and save in the database, must be >= to training size
patch_size2 = 708 #size of the tiles to extract and save in the database, must be >= to training size
...
files = glob.glob("C:/Users/Burak/Desktop/asd/masks/*.png") # create a list of the files, in this case we're only interested in files which have masks so we can use supervised learning
...
block_shape["img"] = np.array((patch_size2, patch_size, 3))
block_shape["mask"] = np.array((patch_size2, patch_size))
    ...
    for filei in phases[phase]:  # now for each of the files
        ...
        for imgtype in imgtypes:
            if (
                ...
                io = cv2.cvtColor(cv2.imread("C:/Users/Burak/Desktop/asd/" + os.path.basename(fname).replace("_mask.png", ".png")),
                                  cv2.COLOR_BGR2RGB)
            ...
            io_arr_out = sklearn.feature_extraction.image.extract_patches(io, (patch_size2, patch_size, 3), stride_size)

            # resize it into a ntile x patch_size x patch_size x 3
            io_arr_out = io_arr_out.reshape(-1, patch_size2, patch_size, 3)
